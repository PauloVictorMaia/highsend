...
import { InputConfig, InputPreview, NodeContainer, MenuInput, MenuButton, CustomToolbar, CloseButton } from "./cpf.style";
...
export function Cpf({ data, id, groupID }) {
  const { setNodes, deleteElements } = useReactFlow();
  const { createNewVariable, variables, nodeMenuIsOpen, setNodeMenuIsOpen } = useStateContext();
  const [newVariable, setNewVariable] = useState("");
  const [question, setQuestion] = useState(data.question || "Qual é o seu CPF?");
  const [description, setDescription] = useState(data.description || "");
  const [placeholder, setPlaceholder] = useState(data.placeholder || "000.000.000-00");
